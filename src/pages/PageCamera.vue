<template>
  <q-page class="constrain-more q-pa-md">
    ...
    <div class="text-center q-pa-md">
      ...
      <div class="row justify-center q-ma-md">
        <q-input :loading="locationLoading" class="col col-sm-6" v-model="post.location" label="Location" dense>
...
export default defineComponent({
  ...
  methods: {
    ...
    captureImageFallback(event){
      let file = event.target.files[0];
      this.post.photo = file;

      //console.log('ARQUIVO: ' + file);
      //console.log('ARQUIVO TYPE: ' + file);
    ...
    getLocation(){
      ...
      navigator.geolocation.getCurrentPosition(position => {
        console.log('getLocation() 2: ' + position);
        this.getCityAndCountry(position);
      }, err => {
        this.locationError();
        console.log('getLocation() 3:' + err);
      }, {timeout: 7000});
    },
    getCityAndCountry(position){
      ...
      this.$axios.get(apiUrl).then(result => {
        console.log('Localização trazida com sucesso');
        this.locationSuccess(result);
      }).catch(err => {
        this.locationError();
        console.log('Não foi possível trazer a localização');
      });
    },
